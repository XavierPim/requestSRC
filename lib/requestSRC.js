...
class RequestSRC {
    ...
    buildDummyRow({ reqType, city, region, country, userAgent, days, index = 0, total = 1 }) {
        const dayMs = 24 * 60 * 60 * 1000;
        const maxDays = Math.max(3, Math.min(Number(days) || 180, 180));
        const totalRows = Math.max(1, Number.parseInt(total, 10) || 1);
        const safeIndex = Math.max(0, Math.min(Number.parseInt(index, 10) || 0, totalRows - 1));

        // Stratify timestamps across the full range to avoid tight clustering in a short window.
        const rangeMs = maxDays * dayMs;
        const bucketMs = rangeMs / totalRows;
        const ageMs = Math.floor((safeIndex * bucketMs) + (Math.random() * bucketMs));
        ...
        return [
            ...
            country,
            userAgent,
            `requestsrc_dummy_sim:${reqType}`
        ];
    }

    async seedDummyData({ count = 180, days = 180 } = {}) {
        const safeCount = Math.max(1, Math.min(Number.parseInt(count, 10) || 180, 5000));
        const safeDays = Math.max(3, Math.min(Number.parseInt(days, 10) || 180, 180));

        const reqTypes = ['login', 'api', 'checkout'];
        const locations = [
        ...
        for (let i = 0; i < safeCount; i += 1) {
            const reqType = reqTypes[Math.floor(Math.random() * reqTypes.length)];
            const location = locations[Math.floor(Math.random() * locations.length)];
            const userAgent = userAgents[Math.floor(Math.random() * userAgents.length)];
            rows.push(this.buildDummyRow({
                reqType,
                city: location.city,
        ...
        for (let start = 0; start < rows.length; start += chunkSize) {
            ...
            await database.query(
                `INSERT INTO logs (timestamp, ip, city, region, country, user_agent, req_type) VALUES ${valuesSql};`,
                params
            );
        }
    ...
    async deleteDummyData() {
        const result = await database.query(
            "DELETE FROM logs WHERE req_type LIKE 'requestsrc_dummy_sim:%';"
        );
    ...
    setupRoutes() {
        ...
        this.router.post(
            `${dashboardRoute}/dummy-data`,
            (req, res, next) => this.requireDashboardToken(req, res, next),
            async (req, res) => {
                try {
                    const count = Number.parseInt(req.query.count, 10);
                    const days = Number.parseInt(req.query.days, 10);
                    const inserted = await this.seedDummyData({
                        count: Number.isInteger(count) ? count : 180,
                        days: Number.isInteger(days) ? days : 180
                    });
        ...
        this.router.delete(
            ...
            async (req, res) => {
                try {
                    const deleted = await this.deleteDummyData();
                    res.json({ deleted });
                } catch (error) {
